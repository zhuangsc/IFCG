...
static int _cs_contains(const cs *A, int k, int i0)
{
	int *Ap = A->p;
	int *Ai = A->i;
	int p, i;
...
csn *cs_chol_ilu (const cs *A, const css *S)
{
    double d, lki, *Lx, *x ;
    int top, i, p, k, n, *Li, *Lp, *cp, *Pinv, *w, *s, *c, *parent ;
    cs *L, *C, *E ;
...
{
    int ok;
    if (!b) return (0) ;		/* check inputs */
    double *tmp = cs_malloc (n, sizeof (double)) ;
    ok = (S && N && x) ;
    if (ok)
    {
		cs_ipvec (n, S->Pinv, b, tmp) ;	/* x = P*b */
		cs_lsolve (N->L, tmp) ;		/* x = L\x */
		cs_ltsolve (N->L, tmp) ;		/* x = L'\x */
		cs_pvec (n, S->Pinv, tmp, x) ;	/* b = P'*x */
    }
	cs_free(tmp);
    return (ok) ;
}
